...
class MSPS():
    ...
    def make_patch(self, U, k=5):
        batch_size, channels, height, width = U.shape

        # Calculate patch size
        patch_height = (height + k - 1) // k
        patch_width = (width + k - 1) // k

        # Pad the input tensor to handle edge cases
        padded_input = F.pad(U, (0, (patch_width * k - width), 0, (patch_height * k - height)),
        ...
        patches = patches.contiguous().view(batch_size, channels, k * k, patch_height, patch_width)

        return patches

    def flatten_patches(self, patches):
        batch_size, channels, num_patches, patch_height, patch_width = patches.size()

        # Flatten each patch into a 1D vector
        flattened_patches = patches.view(batch_size, channels, num_patches, -1)

        return flattened_patches


    def patch_similarities(self, patches):
        batch_size, channels, num_patches, _ = patches.size()

        # Create an empty tensor to store the cosine similarities
        similarity_matrix = torch.zeros(batch_size, channels, num_patches, num_patches)

        # Compute the cosine similarity between all pairs of patches
        for i in range(num_patches):
            for j in range(num_patches):
                ui = patches[:, :, i, :]  # Select patch i
                uj = patches[:, :, j, :]  # Select patch j
